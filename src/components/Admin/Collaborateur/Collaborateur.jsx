import React, { useState,useEffect } from 'react'
import './Collaborateur.css'
import Layout from '../../layout/Layout'
import MaterialTable from '@material-table/core'
import collaborateurs from '../../../datas/users'
import axios from 'axios'
export default function Colaborateur() {
  const [users,setUsers] = useState(collaborateurs)
  console.log('collaborateur')

  // useEffect(()=>{
  //   axios.get('http://localhost:8000/api/allUsers',{headers:{
  //     Authorization: 'Bearer '+ localStorage.getItem('token')
  //   }})
  //   .then(res=>console.log(res))
  // },[])
  const columns = [
    { title: 'Profile', field: 'photoProfile',editable:false,
      render: rowData => <img src={ rowData.id? users[rowData.id-1].img : 'data:image/png;base64,'} style={{width: 40,height:40,objectFit:'cover', borderRadius: '50%'}}/>
    },
    { title: 'Nom', field: 'nom' },
    { title: 'Prenom', field: 'prenom' },
    {
      title: 'Departement',
      field: 'departement',
      // lookup: { 34: 'İstanbul', 63: 'Şanlıurfa' },
    },
    {
      title: 'Age',
      field: 'dateNaissance',
    },
    {
      title: 'Shift',
      field: 'horaireTravail',
    },
  ]
  return (
    <div className="collaborateur">

     <MaterialTable style={{width:'100%'}}
      title="Liste Collaborateurs"
      columns={columns}
      data={users}
      editable={{
        onRowAdd:(newRow)=> new Promise((resolve,reject)=>{
          const updateRows=[...users,newRow];
          setUsers(updateRows)
          resolve()
        }),
        onRowDelete:selectedRow=>new Promise((resolve,reject)=>{
          // const updatedTable = [...users]
          // updatedTable.splice(selectedRow.id-1,1)
          // setUsers(updatedTable)
          // setTimeout(()=>resolve(),500)
          // console.log(selectedRow.id-1)
        })
      }}

      options={{
        actionsColumnIndex:-1,addRowPosition:"first",
        selection: true,
        padding: "dense",
        // cellStyle: {textAlign:'center'},
        // headerStyle: {textAlign:'center'}
      }}
      />




    </div>
  )
}

    